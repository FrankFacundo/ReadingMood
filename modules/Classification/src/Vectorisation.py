...
AdresseGitThiziri = ""
AdresseGitClement = ""
userinfo = input("Utilisateur est : Thierry (0)? Thiziri(1)? Clement(2)? ")
if userinfo == "0":
    user = "Thierry"
...
Dirgit = "\\modules\\Classification\\Rapports et autres archives de travail\\stopWords.txt"
DirAbsolu = AdresseGit + Dirgit
fileSW=open(DirAbsolu,"r")
...
def Dico():
    AdresseVocab = AdresseGit + "\\modules\\Classification\\Rapports et autres archives de travail\\Vocabulaire.txt"
    Vocabstr = readtxt()
    Vocab = eval(Vocabstr)
    prepDico = []
    for mot in Vocab:
...
def recuperationOfDataSet():
    if user == "Thierry":
        adresseDataSet = "C:\\Users\Thierry\Documents\Telecom ParisTech\PACT 3.5\Data Set"
    
    elif user == "Thiziri":
        print("Adresse de Thiziri non renseignée, voir recuperationOfDataSet")
        adresseDataSet = ""
        return None
        
    elif user == "Clement":
        print("Adresse de Clement non renseignée, voir recuperationOfDataSet")
        adresseDataSet = ""
        return None
    return adresseDataSet

def VectorizationOfDataSet():   # La liste de tous les textes
    corpus = []
    adresseDocAmb = recuperationOfDataSet() 
        
    LAmbiances = os.listdir(adresseDocAmb)
    
    for ambiance in LAmbiances:
        PtrAmbFold = os.listdir(adresseDocAmb + "\\" + ambiance)
        
        for PtrFile in PtrAmbFold:

            AdresseText = PtrAmbFold + "\\" + PtrFile
...
"""
Ces fonctions sont notamment adaptées à l'interface de Thierry Bécart et n'ont pas été adaptées avec git"
...
AdresseRelVocab = "\\modules\\Classification\\Rapports et autres archives de travail\\Vocabulaire.txt"
AdresseVocab = AdresseGit + AdresseRelVocab
